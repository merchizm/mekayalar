    ...
    return (
        <Link href={route('blog.show', { slug: post.post_slug })} className="group mx-auto block w-full max-w-4xl">
            <div className="surface-lift relative overflow-hidden rounded-3xl border border-amber-200 bg-amber-50/70 p-8 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-amber-900 dark:bg-amber-950/20">
                ...
                <blockquote className="relative border-l-4 border-amber-400 pl-5 text-2xl font-semibold leading-relaxed text-foreground transition duration-300 group-hover:translate-x-1 dark:text-foreground">
                    “{post.quote_text || post.description || post.post_title}”
                </blockquote>
                <div className="mt-5 flex flex-wrap items-center gap-3 text-sm text-muted-foreground dark:text-muted-foreground">
                ...
                {post.description && (
                    <p className="mt-4 text-sm text-muted-foreground transition duration-300 group-hover:text-foreground dark:text-muted-foreground dark:group-hover:text-foreground">
                        {post.description}
                    </p>
                )}
            </div>

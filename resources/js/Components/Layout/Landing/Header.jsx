        ...
        <nav className={`mobile-nav-wrapper overflow-hidden rounded-full backdrop-blur-lg bg-background/80 dark:bg-background-dark/80 ${scrolled ? 'ring-1 shadow-lg ring-black/5' : ''}`}>
          <div className="mobile-nav-container">
            <ul className="flex gap-1 items-center p-2 whitespace-nowrap">
              <li><NavLink href={route('landing.index')} active={url === '/'} loading={isNavigating(route('landing.index'))}>{__('Hakkımda')}</NavLink></li>
              <li><NavLink href={route('blog.index')} active={url.startsWith('/posts')} loading={isNavigating(route('blog.index'), true)}>{__('Gönderilerim')}  </NavLink></li>
              <li><NavLink href={route('poems.index')} active={url.startsWith('/poems')} loading={isNavigating(route('poems.index'), true)}>{__('Şiirlerim')}</NavLink></li>
              <li><NavLink href={route('projects.index')} active={url.startsWith('/projects')} loading={isNavigating(route('projects.index'), true)}>{__('Projelerim')}</NavLink></li>
              <li><NavLink href={route('bookshelf.index')} active={url.startsWith('/bookshelf')} loading={isNavigating(route('bookshelf.index'), true)}>{__('Kitaplık')}</NavLink></li>
              <li><NavLink href={route('bookmarks.index')} active={url.startsWith('/bookmarks')} loading={isNavigating(route('bookmarks.index'), true)}>{__('Yer İmleri')}</NavLink></li>
            </ul>
          </div>
        </nav>
      </div>

...
export default function ScreenSaver() {
  ...
  `;
    ...
    return (
        <div
            className={`fixed inset-0 z-[100] flex flex-col items-center justify-center bg-background text-[white] transition-opacity duration-500 dark:bg-background ${showScreenSaver ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
        >
            <style>{animationStyles}</style>
            <span
                key={currentText}
                className="text-animate text-[14vmin] font-medium tracking-[2px] text-foreground dark:text-foreground"
            >
                {currentText}
            </span>
            <span className="absolute bottom-[6vmin] m-[5px] text-foreground dark:text-foreground">
                {__('Mesajınızı bekliyorum')}
            </span>
            <a
                className="absolute bottom-[4vmin] m-[5px] font-medium text-muted-foreground no-underline dark:text-muted-foreground"

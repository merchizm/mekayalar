...
export default function SpotifyPlaying() {
  ...
  return (
    ...
    >
      {isLoading ? (
        <div className="flex gap-2 items-center px-3 h-10 rounded-lg transition-colors hover:bg-button-hover dark:hover:bg-button-hover-dark" title={__('Spotify\'da Dinliyorum') + `: ${musicName}`}>
          ...
          {showTooltip && musicUrl && (
            <div className="absolute bottom-full left-1/2 z-10 px-3 py-2 mb-2 w-max text-xs rounded shadow-lg -translate-x-1/2 tooltip-bubble text-default dark:text-white bg-background dark:bg-repository-card-bg-dark">
              <div className="flex gap-4 items-center">
                <a href={musicUrl} target="_blank" rel="noopener noreferrer" className="hover:text-spotify-green">{__('Müziği aç')}</a>
                <a href="https://open.spotify.com/user/hkt7thwkuynqutz8jenb3x0wu?si=eb716f20515241b4" target="_blank" rel="noopener noreferrer" className="hover:text-spotify-green">{__('Profilime git')}</a>
              </div>
            </div>
          )}
        </div>
      ) : (
        <div className="flex justify-center items-center w-10 h-10 rounded-lg transition-colors" title={__('Spotify\'da aktif değilim')}>

...
export default function Index({ auth, books = [] }) {
    ...
    const deleteBook = (book) => {
        if (!confirm(__('Kitap silme onayı: ":title" kitabını silmek istediğinizden emin misiniz?', { title: book.title }))) {
            return;
        }
        ...
        <AdminLayout user={auth.user}>
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4 dark:border-gray-700">
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{__('Kitaplar')}</h3>
                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{__('Toplam :count kitap', { count: books.length })}</p>
                    </div>
                    <Link
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">{__('Kitap')}</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">{__('Durum')}</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">{__('Puan')}</th>
                                    <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300">{__('İşlemler')}</th>
                                </tr>
                            </thead>

  ...
  return (
    <AdminLayout user={auth.user}>
      ...
      <div className="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link href={route('admin.projects.index')} className="inline-flex justify-center items-center px-4 py-2 font-semibold text-gray-700 bg-gray-200 rounded-lg shadow-sm transition duration-150 ease-in-out hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600">
          Geri Dön
        </Link>
      </div>

      <form onSubmit={submit} className="bg-white rounded-lg border border-gray-200 shadow-sm dark:bg-gray-800 dark:border-gray-700">
        <div className="p-5">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <div className="space-y-6 md:col-span-1">
              ...
              <div className="flex items-center">
                <input id="is_featured" name="is_featured" type="checkbox" className="w-4 h-4 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500" checked={data.is_featured} onChange={e => setData('is_featured', e.target.checked)} />
                <label htmlFor="is_featured" className="block ml-2 text-sm text-gray-900 dark:text-gray-300">Öne Çıkan Proje</label>
              </div>

              <div className="flex items-center">
                <input id="is_published" name="is_published" type="checkbox" className="w-4 h-4 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500" checked={data.is_published} onChange={e => setData('is_published', e.target.checked)} />
                <label htmlFor="is_published" className="block ml-2 text-sm text-gray-900 dark:text-gray-300">Yayınla</label>
              </div>
            </div>
          </div>
        </div>
        <div className="flex justify-end px-5 py-4 border-t border-gray-200 dark:border-gray-700">
          <button type="submit" className="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md border border-transparent shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50" disabled={processing}>Projeyi Kaydet</button>
        </div>
      </form>
    </AdminLayout>
  );

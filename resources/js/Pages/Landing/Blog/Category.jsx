            ...
            <div className="mb-10 flex flex-wrap items-center justify-center gap-4">
                ...
                {categories.map((category) => {
                    ...
                    const inactiveClasses =
                        'bg-button dark:bg-button-dark text-text dark:text-text-dark hover:bg-button-hover dark:hover:bg-button-hover-dark';
                    return (
                        <Link
                            key={category.id}
                            href={route('blog.category', { slug: category.slug })}
                            className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${isActive ? activeClasses : inactiveClasses}`}
                        >
                            {category.name}
                        </Link>
                    );

    ...
    <>
      ...
      <div className="flex flex-wrap gap-4 justify-center items-center mb-10">
        ...
        {categories.map(category => {
          ...
          const inactiveClasses = 'bg-button dark:bg-button-dark text-text dark:text-text-dark hover:bg-button-hover dark:hover:bg-button-hover-dark';
          return (
            <Link key={category.id} href={route('blog.category', { slug: category.slug })} className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${isActive ? activeClasses : inactiveClasses}`}>
              {category.name}
            </Link>
          )
        })}
      ...
      <div className="space-y-12">
        {postItems.length > 0 ? (
          ...
          <div className="py-24 my-5 text-center rounded-2xl border-2 border-dashed bg-background dark:bg-repository-card-bg-dark border-divider dark:border-divider-dark">
            ...
            <h2 className="mb-3 text-3xl font-bold text-text dark:text-text-dark">Bu Kategoride Henüz Gönderi Yok</h2>
            <p className="text-xl text-center text-light-text dark:text-light-text-dark">Bu konu hakkında yazacaklarım olduğunda tekrar uğrayın.</p>
          </div>
        )}
      </div>

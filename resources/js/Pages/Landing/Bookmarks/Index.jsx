  ...
  return (
    <>
      <header className="mb-12 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-xl text-light-text dark:text-light-text-dark">{__('İlham veren, yol gösteren ve ufkumu genişleten dijital duraklarım. Burada seninde hoşuna gidecek şeyler bulabilirsin.')}</p>
      </header>
      ...
      <div className="mx-auto max-w-3xl">
        {sortedBookmarkKeys.length > 0 ? (
          ...
          <div className="py-24 my-5 text-center rounded-2xl border-2 border-dashed bg-background dark:bg-repository-card-bg-dark border-divider dark:border-divider-dark">
            ...
            <h2 className="mb-3 text-3xl font-bold text-text dark:text-text-dark">{__('Henüz Yer İmi Eklenmemiş')}</h2>
            <p className="text-xl text-light-text dark:text-light-text-dark">{__('Keşfettiğim faydalı bağlantıları burada paylaşacağım.')}</p>
          </div>
        )}
      </div>
    </>

  ...
  return (
    <>
      <header className="mb-12 text-center">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-xl text-light-text dark:text-light-text-dark">{__('Geliştirdiğim, katkıda bulunduğum ve gurur duyduğum çalışmalarım.')}</p>
      </header>

      <div className="container space-y-16">
        {featuredProjects.length > 0 && (
          <section>
            <h2 className="mb-6 text-3xl font-bold text-center text-text dark:text-text-dark">{__('Öne Çıkanlar')}</h2>
        ...
        {projects.length > 0 && (
          <section>
            <h2 className="mb-6 text-3xl font-bold text-center text-text dark:text-text-dark">{featuredProjects.length > 0 ? __('Diğer Projeler') : ''}</h2>
        ...
        {featuredProjects.length === 0 && projects.length === 0 && (
          <div className="py-24 my-5 text-center rounded-2xl border-2 border-dashed bg-background dark:bg-repository-card-bg-dark border-divider dark:border-divider-dark">
            ...
            <h2 className="mb-3 text-3xl font-bold text-text dark:text-text-dark">{__('Henüz Proje Eklenmemiş')}</h2>
            <p className="text-xl text-center text-light-text dark:text-light-text-dark">{__('Yakında burada projelerimi görebileceksiniz.')}</p>
          </div>
        )}
      </div>
    </>

...
@section('content')
    <div class="flex gap-[0.5em] items-center justify-between mt-[0.4em] mb-[2em]">
        <div class="flex gap-[0.5em]">
            <button class="flex bg-button-hover dark:bg-button-hover-dark  cursor-pointer px-[1em] py-[0.3em] rounded-md hover:bg-button dark:hover:bg-button-dark">
                📸
            </button>
            <button class="flex bg-button-hover dark:bg-button-hover-dark  cursor-pointer px-[1em] py-[0.3em] rounded-md hover:bg-button dark:hover:bg-button-dark">
                👾
        ...
        <div class="flex gap-[0.5em]">
            ...
            <a class="flex bg-button-hover dark:bg-button-hover-dark  cursor-pointer px-[1em] py-[0.3em] rounded-md hover:bg-button dark:hover:bg-button-dark" href="">
                {{ $category->name }}
            </a>
            @endforeach
        </div>
    ...
    @foreach ($posts as $post)
        @if ($post->type === 'post')
        <a class="flex flex-row gap-[2em] transition-[outline-color] duration-[ease] delay-150 mb-[1em] p-[5px] rounded-[10px] outline outline-1 outline-transparent hover:outline-outline-color dark:hover:outline-outline-color-dark" href="{{  route('blog.show', ['slug' => $post->post_slug]) }}">
            <picture>
                ...
                <img class="h-[300px] transition-[outline-color] duration-[ease] delay-500 rounded-[5px] outline-[hsla(0,0%,100%,0.18)] hover:outline-[hsla(0,0%,100%,0.35)]"/>
            </picture>
            <div class="flex flex-col gap-[1em] w-[45vw]">
                <div class="flex flex-row justify-between">
                    <h4 class="text-[2.2em] leading-[2em]">{{ $post->post_title }}</h4>
                    </div>
                <div>
                ...
                <p class="text-[1.2em] leading-[1em] text-dark-text dark:text-dark-text-dark whitespace-pre-wrap overflow-hidden"><span>{{ Str::limit(strip_tags($post->post_content), 400, '...') }}</span></p>
            </div>
        </a>
        @else
        <div class="relative flex flex-row gap-[2em] dark:bg-[rgba(17,17,19,0.5)] bg-button-hover justify-center transition-all duration-[ease-in-out] delay-[600ms] mb-[1em] p-5 rounded-[20px] outline-1 outline outline-outline-color dark:outline-outline-color my-3">
            ...
            <picture>
                <source media="(max-width: 799px)" srcset="https://images.pexels.com/photos/1624496/pexels-photo-1624496.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" />
                <source media="(min-width: 800px)" srcset="{{ $post->post_image }}" />
                <img class="h-[400px] transition-[outline-color] duration-[ease] delay-500 rounded-[10px] outline outline-1 outline-[hsla(0,0%,100%,0.18)] hover:outline-[hsla(0,0%,100%,0.35)]"/>
            </picture>
        </div>
        @endif
    @endforeach

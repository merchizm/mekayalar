...
@section('content')
    <header class="container mb-8 overflow-hidden">
        <div class="flex items-center gap-5">
            <div class="w-full">
                <h1 class="mb-3 text-4xl font-bold">{{ $typeLabel }} Gönderileri</h1>
                <p class="mb-5 text-lg text-light-text dark:text-light-text-dark">{{ $typeLabel }} türündeki tüm gönderilerim.</p>
            </div>
        </div>
    </header>
    ...
    <div class="flex flex-wrap justify-between gap-3 mb-8">
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('blog.type', ['type' => 'photo']) }}" class="inline-flex items-center px-3 py-2 {{ $currentType == 'photo' ? 'bg-repository-card-bg dark:bg-menu-hover-dark' : 'bg-button dark:bg-button-dark' }} text-text dark:text-text-dark rounded-lg hover:bg-button-hover dark:hover:bg-button-hover-dark transition-colors">
                <span class="mr-2 text-lg">📸</span>
                <span>Fotoğraflar</span>
            </a>
            <a href="{{ route('blog.type', ['type' => 'drawing']) }}" class="inline-flex items-center px-3 py-2 {{ $currentType == 'drawing' ? 'bg-repository-card-bg dark:bg-menu-hover-dark' : 'bg-button dark:bg-button-dark' }} text-text dark:text-text-dark rounded-lg hover:bg-button-hover dark:hover:bg-button-hover-dark transition-colors">
                <span class="mr-2 text-lg">👾</span>
                <span>Çizimler</span>
            </a>
        </div>
        <div class="flex flex-wrap gap-2">
            ...
            <a class="inline-flex items-center px-3 py-2 bg-social-bg dark:bg-social-bg-dark text-text dark:text-text-dark rounded-lg hover:bg-social-bg-hover dark:hover:bg-social-bg-hover-dark transition-colors" href="{{ route('blog.category', ['slug' => $category->slug]) }}">
                {{ $category->name }}
            </a>
            @endforeach
        </div>
    ...
    <div class="py-16 my-5 text-center bg-poem-container dark:bg-poem-container-dark rounded-xl shadow-sm border border-divider dark:border-label-border-dark">
        ...
        <h2 class="mb-3 text-2xl font-semibold text-text dark:text-text-dark">Bu türde henüz gönderi bulunmuyor</h2>

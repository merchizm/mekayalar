...
@section('content')
    <header class="mb-12 text-center">
        <h1 class="text-5xl font-bold tracking-tight text-text dark:text-text-dark">{{ $currentCategory->name }}</h1>
        <p class="mx-auto mt-4 max-w-2xl text-xl text-light-text dark:text-light-text-dark">{{ $currentCategory->description }}</p>
    </header>
    ...
    <div class="flex flex-wrap gap-4 justify-center items-center mb-10">
        ...
        <a href="{{ route('blog.type', ['type' => 'photo']) }}" class="flex gap-2 items-center px-4 py-2 text-sm font-semibold rounded-full border transition-colors bg-button dark:bg-button-dark text-text dark:text-text-dark border-divider dark:border-divider-dark hover:bg-button-hover dark:hover:bg-button-hover-dark hover:border-menu-active/50">
            <span class="text-lg">📸</span>
            <span>Fotoğraflar</span>
        </a>
        <a href="{{ route('blog.type', ['type' => 'drawing']) }}" class="flex gap-2 items-center px-4 py-2 text-sm font-semibold rounded-full border transition-colors bg-button dark:bg-button-dark text-text dark:text-text-dark border-divider dark:border-divider-dark hover:bg-button-hover dark:hover:bg-button-hover-dark hover:border-menu-active/50">
            <span class="text-lg">👾</span>
            <span>Çizimler</span>
        </a>
        <div class="mx-2 w-px h-6 bg-divider dark:bg-divider-dark"></div>
        @foreach ($categories as $category)
        <a class="px-4 py-2 text-sm font-semibold transition-colors rounded-full {{ $currentCategory->id == $category->id ? 'bg-menu-active text-white dark:text-text-dark' : 'bg-button dark:bg-button-dark text-text dark:text-text-dark hover:bg-button-hover dark:hover:bg-button-hover-dark' }}" href="{{ route('blog.category', ['slug' => $category->slug]) }}">
            {{ $category->name }}
        </a>
        @endforeach
    </div>
    ...
    <div class="space-y-12">
        @forelse ($posts as $post)
            ...
            @else
                <div class="mx-auto max-w-2xl text-center">
                    <a href="{{ $post->post_image }}" data-fslightbox="gallery-{{ $currentCategory->slug }}" class="block group">
        ...
        @empty
            <div class="py-24 my-5 text-center rounded-2xl border-2 border-dashed bg-background dark:bg-repository-card-bg-dark border-divider dark:border-divider-dark">
                ...
                <h2 class="mb-3 text-3xl font-bold text-text dark:text-text-dark">Bu Kategoride Henüz Gönderi Yok</h2>
                <p class="text-xl text-center text-light-text dark:text-light-text-dark">Bu konu hakkında yazacaklarım olduğunda tekrar uğrayın.</p>
            </div>
        @endforelse
    </div>
@endsection
